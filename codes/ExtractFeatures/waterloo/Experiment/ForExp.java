...
public class ForExp {
	...
	String crec = "crecFeature-new";
	String wangwei = "wangwei-new+type3";
	String suffix = ".arff";
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 1) {
			...
			outputBasePath = basePath + "finalTest/Results/";
			...
			if (j == 0 || j == 1) {
				...
				for (int i = 0; i < projectName.length; i++) {
					...
					for (int cnt = 1; cnt <= 10; cnt ++) {
						trainName[i] = featureBasePath + "testWithinProjects/" + projectName[i] + "/trainInTen_" + cnt + ".arff";
						testName[i] = featureBasePath + "testWithinProjects/" + projectName[i] + "/testInTen_" + cnt + ".arff";
						w.write("*******************************" + "\n");
				...
				for (int i = 0; i < projectName.length; i++) {
					...
					BufferedWriter w = new BufferedWriter(new FileWriter(new File(outputPath)));
					for (int cnt = 1; cnt <= 10; cnt ++) {
						trainName[i] = featureBasePath + "testWithinProjects/" + projectName[i] + "/" + wangwei + "TrainInTen_" + cnt + ".arff";
						testName[i] = featureBasePath + "testWithinProjects/" + projectName[i] + "/" + wangwei + "TestInTen_" + cnt + ".arff";
						w.write("*******************************" + "\n");
						w.write("Train:" + trainName[i] + "\n");
						w.write("Test :" + testName[i] + "\n");
						String trainFileName = trainName[i];
	...
	public void testFeature(String outputBasePath) throws Exception {
		...
		for (int j = 0; j < 5; j++) {
			...
			for (int i = 0; i < projectName.length; i++) {
				...
				BufferedWriter w = new BufferedWriter(new FileWriter(new File(outputPath)));
				for (int cnt = 1; cnt <= 10; cnt ++) {
					trainName[i] = featureBasePath + "testWithinProjects/" + projectName[i] + "/" + features[j] + "TrainInTen_" + cnt + ".arff";
					testName[i] = featureBasePath + "testWithinProjects/" + projectName[i] + "/" + features[j] + "TestInTen_" + cnt + ".arff";
					w.write("*******************************" + "\n");
					w.write("Train:" + trainName[i] + "\n");
					w.write("Test :" + testName[i] + "\n");
					String trainFileName = trainName[i];
