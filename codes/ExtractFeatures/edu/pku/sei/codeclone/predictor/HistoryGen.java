...
public class HistoryGen {
	...
	boolean isOriginOrNot = false;
	
	public History getHistory(MyFragment frag, HashMap<String, Date> allReponameDateTable, 
			Vector<String> filterCommits, Vector<String> allCommits, String projectPath, String tmpFolderPath) {
		...
		if (frag.versionRepoID == null || frag.versionRepoID.length() == 0) {
			System.out.println("hello:" + frag.getFilePath());
		}
		if (frag.versionRepoID.length() == 5) {
			curVersion = filterCommits.get(Integer.parseInt(frag.versionRepoID) - 1);
			isOriginOrNot = true;
		}
		else 
			curVersion = frag.versionRepoID;
	...
	private HistoryFrag getPred(HistoryFrag currentFrag, String curVersion) {
		...
		if (isOriginOrNot == true) {
			currentPath = currentFrag.filePath.substring(this.projectPath.length() + "Filter/00001/".length());
		}
		else {
			currentPath = currentFrag.filePath.substring(tmpFolderPath.length() + currentFrag.versionRepoID.length() + 1);
			//System.out.println("origin path:" + currentFrag.filePath);
			//System.out.println("ID:" + currentFrag.versionRepoID);
			//System.out.println(currentPath);
		}
		ByteArrayOutputStream out = NewRefactorCommitLocator.read(projectPath, parentHash, currentPath);
		if (out == null || out.size() == 0)
			return null;
		return new HistoryFrag(-1, currentFrag.versionRepoID, -1, currentFrag.filePath, -1, -1, true, parentHash);
	}
